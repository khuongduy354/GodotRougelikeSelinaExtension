...
var previous_player_position: Vector2i
...
func _init(map_width: int, map_height: int, player: Entity) -> void:
	...
	height = map_height
	self.player = player
	previous_player_position = player.grid_position
	entities = []
	_setup_tiles()
...
func get_nearest_enemy() -> Entity: 
	var nearest_enemy = null 
	
	var enemies_in_view = []
	for entity in entities:
		var is_tile_in_view = get_tile(entity.grid_position).is_in_view
		if !is_tile_in_view: 
			continue
			
		var tile_actor =  get_actor_at_location(entity.grid_position)
		if tile_actor and tile_actor != player: 
			enemies_in_view.append(entity)
	
	if enemies_in_view.size() == 0: 
		return null 
	# find the nearest from player.grid_position to enemies_pos 
	# using A* algos
	var nearest_distance = null
	nearest_enemy = enemies_in_view[0]
	for enemy in enemies_in_view: 
		var distance = pathfinder.get_id_path(player.grid_position,enemy.grid_position).size()
		if !nearest_distance: 
